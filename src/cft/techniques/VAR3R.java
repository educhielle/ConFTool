...
public class VAR3R
{
    ...
    private static List<List> errorList = new ArrayList<>();
    ...
    private static final String LOWER_HALF_ADDRESS = "61440"; //0xFFBF
        ...
        for (int i = 0; i < code.size(); i++)
        {
            if (DB.isInstruction(code.get(i)))
            {
                ...
                if (!rd.contains(DB.getSignatureRegister()) && !rs.contains(DB.getSignatureRegister()) && !Util.containsAtLeastOne(freeRegisters, rd) && !Util.containsAtLeastOne(freeRegisters, rs))
                {
                    ...
                    List<String> offsetCopy2 = getCopyOffset(offsetCopy);
                    ...
                    if (DB.isArithmetic(instruction))
                    {
                        ...
                        /* REMOVED -- there is no need to check before arithmetic instructions
                    ...
                    else if (DB.isLoad(instruction))
                    {
                        ...
                        for (int j = 0; j < rsIntercalate.size(); j++)
                        {
                            ...
                            //Insert label after branch to jump back from recovery
                            code.add(i++, DB.getLabelFormat().replace("label", getErrorLabelName(rsIntercalate.get(j)).get(0) + "_return"));
                            VAR3R.errorCounter++;
                        ...
                        if (!rd.equals(rdCopy))
                        {
                            /* REMOVED -- it is not necessary in the new methodology
                            i = prepareCopyAddressRegisters(code, ++i, rsCopy);
                            i--;
                            */
                            
                            code.add(++i, Util.generateCommand(instruction.getName(), rdCopy, rsCopy, emptyList, offsetCopy, emptyList, instruction.getFormat()));
                            
                            //Check loaded values
                            //cmp ro, rc
                            if (DB.hasComparisonInstruction())
                            {
                                code.add(++i, Util.generateCommand(cmp.getName(), emptyList, rdIntercalate.get(0), emptyList, emptyList, emptyList, cmp.getFormat()));
                            }
                            //beq ok
                            code.add(++i, Util.generateCommand(beq.getName(), emptyList, rdIntercalate.get(0), emptyList, emptyList, Util.createList("load_"+loadCounter+"_ok"), beq.getFormat()));
                            
                            //ldr r3, [rca, #offsetCopy2]
                            code.add(++i, Util.generateCommand(instruction.getName(), addressRegisterList, rsCopy, emptyList, offsetCopy2, emptyList, instruction.getFormat()));
                            
                            //cmp rc, r3
                            List<String> listRcR3 = Util.createList(rdCopy.get(0), addressRegisterList.get(0));
                            if (DB.hasComparisonInstruction())
                            {
                                code.add(++i, Util.generateCommand(cmp.getName(), emptyList, listRcR3, emptyList, emptyList, emptyList, cmp.getFormat()));
                            }
                            //bne else
                            code.add(++i, Util.generateCommand(bne.getName(), emptyList, listRcR3, emptyList, emptyList, Util.createList("load_"+loadCounter+"_else"), bne.getFormat()));
                            
                            //str rc, [roa, #offset]
                            code.add(++i, Util.generateCommand(store.getName(), emptyList, Util.createList(rdCopy.get(0), rs.get(0)), emptyList, offset, emptyList, store.getFormat()));
                            ...
                            //b ok
                            code.add(++i, Util.generateCommand(jump.getName(), emptyList, emptyList, emptyList, emptyList, Util.createList("load_"+loadCounter+"_ok"), jump.getFormat()));
                            
                            //else:
                            code.add(++i, DB.getLabelFormat().replace("label", "load_"+loadCounter+"_else"));
                            
                            //str ro, [rca, #offsetCopy]
                            code.add(++i, Util.generateCommand(store.getName(), emptyList, Util.createList(rd.get(0), rsCopy.get(0)), emptyList, offsetCopy, emptyList, store.getFormat()));
                            ...
                            //ok:
                            code.add(++i, DB.getLabelFormat().replace("label", "load_"+loadCounter+"_ok"));
                            loadCounter++;
                            

                            //***TEMPORARILY REMOVED -- it is necessary to adapt to new methodology
                            //i = loadTripleCheck(code, ++i, rd, rdCopy, rs, rsCopy, offset);
                            //i--;
                            
                            /* REMOVED -- it will not be necessary after modifying loadTripleCheck
                    ...
                    else if (DB.isStore(instruction))
                    {
                        ...
                        if (!rs.equals(rsCopy))
                        {
                            /* Removed because it is not necessary in the new methodology
                            ...
                            */
                            ...
                            //***TEMPORARILY REMOVED
                            code.add(++i, Util.generateCommand(instruction.getName(), emptyList, rsThird, emptyList, offsetCopy2, emptyList, instruction.getFormat()));
                            
                            /* REMOVED -- Not necessary in the new methodology
                    ...
                    for (int j = 0; j < hiddenRD.size(); j++)
                    {
                        ...
                        if (!newRD.equals(newRS))
                        {
                            ...
                            /* REMOVED -- Unnecessary in the new methodology
                            for (int k = 0; k < usedRegisters.size(); k++)
                            {
                                List<String> ur = Util.createList(usedRegisters.get(k));
                                List<String> cr = Util.createList(copyRegisters.get(k));
                                code.add(++i, Util.generateCommand(move.getName(), cr, ur, emptyList, emptyList, emptyList, move.getFormat()));
                            }
                            */
    ...
    public static List<String> getErrorLabelName(List<String> register)
    {
        ...
        try
        {
            ...
            list.add(DB.ERROR_LABEL_NAME + VAR3R.errorCounter);
        }
        catch (Exception error)
        {
    ...
    */
    ...
    private static List<String> insertErrorRecoverySubroutines(List<String> code)
    {
        ...
        {
            code = insertErrorRecoverySubroutine(code, i);
        }
        ...
        */
        ...
        /*
        for (int i = 0; i < VAR3R.errorCounter; i++)
        {
            code.add(DB.ERROR_LABEL_NAME+i+":");
            code.add(   "	movw	r0, #:lower16:g_mss_uart1\n" +
                        "	movt	r0, #:upper16:g_mss_uart1\n" +
                        "	mov	r1, #115200\n" +
                        "	mov	r2, #3\n" +
                        "	bl	MSS_UART_init\n" +
                        "	add	r3, r7, #4\n" +
                        ...
                        "	mov	r0, r3\n" +
                        "	mov	r1, #56\n" +
                        "	movw	r2, #:lower16:.LCerror"+i+"\n" +
                        "	movt	r2, #:upper16:.LCerror"+i+"\n" +
                        "	bl	snprintf\n" +
                        "	add	r3, r7, #4\n" +
                        "	movw	r0, #:lower16:g_mss_uart1\n" +
                        "	movt	r0, #:upper16:g_mss_uart1\n" +
                        "	mov	r1, r3\n" +
                        "	mov	r2, #56\n" +
                        "	bl	MSS_UART_polled_tx\n" +
                        "	b	loop"
                    );
        }
        ...
        //int j;
        //for (j = 0; !code.get(j).equals(".LC0:"); j++);
        
        for (int i = 0; i < VAR3R.errorCounter; i++)
        {
            code.add(   ".LCerror"+i+":\n" +
                        "	.ascii	\"#### ERROR "+i+"\\012\\015\\000\"\n" +
                        "	.align	2");
        }
        */
    ...
    private static List<String> insertErrorRecoverySubroutine(List<String> code, int ec)
    {
        String offset, copyRegister, register = (String) errorList.get(ec).get(0);
        try
        {
            ...
            List<String> errorLabelNameList = Util.createList(DB.ERROR_LABEL_NAME + ec);
            ...
            //B errorN_return
            String targetN_return = errorLabelNameList.get(0) + "_return";
            List<String> targetN_returnList = Util.createList(targetN_return);
            code.add(Util.generateCommand(jump.getName(), emptyList, emptyList, emptyList, emptyList, targetN_returnList, jump.getFormat()));
