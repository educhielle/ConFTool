...
public class VAR3pRESO
{
    ...
    {
        String fileNameOut = Util.getFileTitle(fileNameIn) + "VAR3pRESO" + RESO_SHIFT + "." + Util.getFileExtension(fileNameIn);
                ...
                if (!rd.contains(DB.getSignatureRegister()) && !rs.contains(DB.getSignatureRegister()) && !Util.containsAtLeastOne(freeRegisters, rd) && !Util.containsAtLeastOne(freeRegisters, rs))
                {
                    ...
                    if (DB.isArithmetic(instruction))
                    {
                        ...
                        if (instruction.equals(add))
                        {
                            ...
                            List<String> rsCopyVisited = new ArrayList<>();
                            for (String copyRegister : rsCopy)
                            {
                                if (!rsCopyVisited.contains(copyRegister))
                                {
                                    ...
                                    rsCopyVisited.add(copyRegister);
                                }
                            }
                        }
                        ...
                        if (instruction.equals(add))
                        {
                            ...
                            for (String copyRegister : rsCopy) if (!shiftRightRegs.contains(copyRegister)) shiftRightRegs.add(copyRegister);
