...
public class VAR2p
{
    ...
    {
        String fileNameOut = Util.getFileTitle(fileNameIn) + "VAR2p." + Util.getFileExtension(fileNameIn);
            ...
            if (DB.isInstruction(code.get(i)))
            {
                ...
                if (!rd.contains(DB.getSignatureRegister()) && !rs.contains(DB.getSignatureRegister()) && !Util.containsAtLeastOne(freeRegisters, rd) && !Util.containsAtLeastOne(freeRegisters, rs))
                {
                    ...
                    if (DB.isArithmetic(instruction))
                    {
                        ...
                        for (int j = 0; j < rdIntercalate.size(); j++)
                        {
                            if (freeComparisonArea && !rdIntercalate.get(j).get(0).equals(rdIntercalate.get(j).get(1)))
                            {
                                if (DB.hasComparisonInstruction())
                                {
                                    code.add(++i, Util.generateCommand(comparison.getName(), emptyList, rdIntercalate.get(j), emptyList, emptyList, emptyList, comparison.getFormat()));
                                }
                                code.add(++i, Util.generateCommand(branchComparison.getName(), emptyList, rdIntercalate.get(j), emptyList, emptyList, errorLabelName, branchComparison.getFormat()));
