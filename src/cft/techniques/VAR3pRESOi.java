 ...
 * @author Eduardo
 */
public class VAR3pRESOi
{
    public static final int RESO_SHIFT = DB.getResoShift();
    public static final int MAX_SHIFT = 4;
    public static List<String> usedRegisters = new ArrayList<String>();
    public static List<String> freeRegisters = new ArrayList<String>();
    public static List<String> copyRegisters = new ArrayList<String>();
    
    public static String apply(String fileNameIn) throws IOException
    {
        String fileNameOut = Util.getFileTitle(fileNameIn) + "VAR3pRESOi" + RESO_SHIFT + "." + Util.getFileExtension(fileNameIn);
    ...
    public static void apply(String fileNameIn, String fileNameOut) throws IOException
    {
        ...
        Instruction add = DB.getAddRegistersInstruction();
        Instruction addi = DB.getAddImmediateInstruction();
        Instruction slli = DB.getShiftLeftImmediateInstruction();
        Instruction srli = DB.getShiftRightImmediateInstruction();
        Instruction srai = DB.getShiftRightArithmeticImmediateInstruction();
        ...
        List<String> registerZero = Util.createList(DB.getRegisterZero().getName());
        List<String> zero = Util.createList("0");
        List<String> resoImm = Util.createList(String.valueOf(RESO_SHIFT));
        ...
        for (int i = 0; i < code.size(); i++)
        {
            if (DB.isInstruction(code.get(i)))
            {
                ...
                if (!rd.contains(DB.getSignatureRegister()) && !rs.contains(DB.getSignatureRegister()) && !Util.containsAtLeastOne(freeRegisters, rd) && !Util.containsAtLeastOne(freeRegisters, rs))
                {
                    ...
                    if (DB.hasComparisonInstruction() && instruction.getName().equals(comparison.getName()))
                    {
                    ...
                    {
                        // apply RESO shift left
                        if (instruction.equals(addi))
                        {
                            //shift = (shift % MAX_SHIFT) + 1;
                            //resoImm = Util.createList(String.valueOf(shift));
                            
                            for (String copyRegister : rsCopy)
                            {
                                List<String> rsC = Util.createList(copyRegister);
                                code.add(++i, Util.generateCommand(slli.getName(), rsC, rsC, resoImm, emptyList, emptyList, slli.getFormat()));
                            }
                            
                            String strImm = imm.get(0);
                            if (strImm.contains("x") || strImm.contains("X"))
                            {
                                strImm = strImm.substring(strImm.indexOf('x')+1);
                                strImm = String.valueOf(Util.hexToDec(strImm));
                            }
                            
                            imm = Util.createList(String.valueOf(Integer.parseInt(strImm)<<RESO_SHIFT));
                        ...
                        // apply RESO shift right
                        if (instruction.equals(addi))
                        {
                            List<String> shiftRightRegs = new ArrayList<>();
                            for (String copyRegister : rsCopy) shiftRightRegs.add(copyRegister);
                            if (!shiftRightRegs.contains(rdCopy.get(0))) shiftRightRegs.add(rdCopy.get(0));
                            
                            for (String rightReg : shiftRightRegs)
                            {
                                List<String> listRightReg = Util.createList(rightReg);
                                code.add(++i, Util.generateCommand(srai.getName(), listRightReg, listRightReg, resoImm, emptyList, emptyList, srai.getFormat()));
                            }
                        }
                    }
                    else if (DB.isLoad(instruction))
                    {
                        ...
                        if (!rd.equals(rdCopy))
                        {
                            code.add(++i, Util.generateCommand(instruction.getName(), rdCopy, rsCopy, imm, offset, target, instruction.getFormat()));
                        }
                    }
                    else if (DB.isLoadFromStack(instruction))
                    {
                        ...
                        for (List<String> currentRD : rdIntercalate)
                        {
                            if (!currentRD.get(0).equals(currentRD.get(1)))
                            {
                                ...
                                code.add(++i, Util.generateCommand(moveInstruction.getName(), currentCopyRD, currentOriginalRD, zero, emptyList, emptyList, moveInstruction.getFormat()));
                            }
                        }
                    }
                    ...
                    else if (DB.isComparison(instruction))
                    {
                        freeComparisonArea = true;
                    ...
                    else if (DB.isOther(instruction))
                    {
                        if (!rd.equals(rdCopy))
                        {
                            code.add(++i, Util.generateCommand(moveInstruction.getName(), rdCopy, rd, zero, emptyList, emptyList, moveInstruction.getFormat()));
                        }
                    }
                    ...
                    for (int j = 0; j < hiddenRD.size(); j++)
                    {
                        ...
                        if (!newRD.equals(newRS))
                        {
                            ...
                            code.add(++i, Util.generateCommand(moveInstruction.getName(), newRD, newRS, zero, emptyList, emptyList, moveInstruction.getFormat()));
                        }
                    }
                ...
                if (DB.getMainLabelName().equalsIgnoreCase(DB.getLabel(code.get(i))))
                {
                    for (int j = 0; j < preInitializedRegisters.size(); j++)
                    {
                        if (usedRegisters.contains(preInitializedRegisters.get(j)))
                        {
                            ...
                            if (!rd.equals(rs))
                            {
                                code.add(++i, Util.generateCommand(moveInstruction.getName(), rd, rs, zero, emptyList, emptyList, moveInstruction.getFormat()));
                            }
                        }
                    }
                }
